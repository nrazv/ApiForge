...
export default function ResourcesTab({ projectId }: ResourcesTabProps) {
  ...
  const [createResourceOpen, setCreateResourceOpen] = useState<boolean>(false);
  const [newResourceName, setNewResourceName] = useState("");
  const [addFieldOpen, setAddFieldOpen] = useState(false);
  const [addFieldResourceId, setAddFieldResourceId] = useState<string | null>(
    null,
  );
    ...
    const fetchResources = async () => {
      ...
      try {
        const response = await fetch(
          `${apiBase}/api/projects/${projectId}/definitions`,
          {
            credentials: "include",
          },
        );
        ...
        for (const model of data ?? []) {
          ...
          mappedFields[modelId] = modelFields.map((field) => ({
            id:
              field.Id ??
              field.id ??
              `${modelId}:${field.Name ?? field.name ?? "field"}`,
  ...
  const handleCreateResource = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const resourceFields: ApiField[] = (data.Fields ?? data.fields ?? []).map(
        (field) => ({
          id:
            field.Id ??
            field.id ??
            `${resourceId}:${field.Name ?? field.name ?? "field"}`,
          name: field.Name ?? field.name ?? "",
          data_type: (field.Type ?? field.type ?? "string") as FieldType,
  ...
  const handleDeleteResource = async (
    ...
    try {
      const response = await fetch(
        `${apiBase}/api/projects/${projectId}/definitions/${resourceId}`,
        {
          method: "DELETE",
          credentials: "include",
        },
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-muted-foreground">
              No resources yet. Create one to define your API.
            </p>
          </CardContent>
        </Card>
      ) : (
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        setConfirmDescription(
                          "This will permanently remove the resource.",
                        );
                    ...
                    {(fields[resource.id] || []).map((field) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <Badge
                            variant="secondary"
                            className={fieldTypeColors[field.data_type]}
                          >
                            {field.data_type}
                          </Badge>
                        </div>
                        <Button
                          ...
                          onClick={() => {
                            ...
                            setConfirmDescription(
                              "This will permanently remove the field.",
                            );
                            setConfirmLabel("Delete");
                            setConfirmAction(
                              () => () =>
        ...
        <DialogContent>
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <Select
                value={addFieldType}
                onValueChange={(value) => setAddFieldType(value as FieldType)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
